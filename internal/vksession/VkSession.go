	...
	for index <= len(CHECKOK) {
		if self.data.data[index] != CHECKOK[ind] {
			...
			self.vk.logs(fm("ERROR! msg = %s\n\tlogin = %s\n\tparams = %v\n\tbody = %s",
				msg, self.vk.Login, self.params, string(self.getByte(100))))
			return false
		}
		index++
		ind++
...
func (self *VkSession) logs(str ...string) {
	...
	self.muGlobal.Lock()
	t := time.Now().Unix()
	logs(fmt.Sprintf("%d > %s %s %s : %s", t, self.Login, self.MyName, self.MyId, content), name)
	self.muGlobal.Unlock()
}
...
func (self *VkSession) logsErr(err error) {
	self.muGlobal.Lock()
	t := time.Now().Unix()
	logs(fmt.Sprintf("%d > %s %s %s : %v", t, self.Login, self.MyName, self.MyId, err), "error")
	self.muGlobal.Unlock()
}
